...
>> "점수 함수" *채점([Situation, Response, T askState] → [Score, F eedback])* 은 확률적일 수 있다.
...
>> 자체 업데이트를 진행하는 *TakeUpdate([Response, T askState] → TaskState)* 는 최근 상황에 대한 응답에 따라 작업 상태를 변이시킨다. 이는 확률적일 수 있다. 
