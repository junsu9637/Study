...
> 이 범위와 관련하여 적용 범위와 자체 예측성을 설명해야 한다. 즉, 타당성을 확립해야 한다. 실제로 벤치마크에서의 성공과 실제 작업 범위의 통계적 관계를 경험적으로 결정함으로써 달성될 것이다.

> 신뢰할 수 있어야 한다. 즉, 재현 가능해야 한다. 평가 시간에 확률적 요소가 포함된 경우, 이러한 요소에 대해 다른 값을 샘플링하는 것은 결과에 의미 있는 영향을 미치지 않아야한다. 벤치마크를 사용하여 동일한 시스템 또는 접근 방식을 독립적으로 평가하는 서로 다른 연구자들이 동일한 결론에 도달해야 한다.

> 광범위한 능력 및 개발자 인식 일반화를 측정하기 시작해야 한다.         
  \- 단순히 스킬이나 잠재력(달성 가능한 최대 기술)만 측정해서는 안된다.        
  \- 시험 응시 시스템 자체 또는 시스템 개발자에 평가 세트의 미리 알려진 작업을 포함해서는 안된다(1.3.2 참조).          
  \- 측정하는 일반화 난이도의 정량화를 추구해야 하고, 최소한 일반화 어려움에 관한 질적 지침을 제공해야 한다. 벤치마크가 지역 일반화(일반성), 광범위한 일반화(유연성), 극단적 일반화(일반 지능)를 측정하려고 하는지 여부를 명확히 해야 한다(1.3.2 참조). 일반화 난이도 고려는 광범위한 능력을 우회하여 지름길(예: 이미지 인식에서 의미론적 내용 대신 표면 질감을 활용하는 것)을 사용하는 사람에 의해 주어진 벤치마크가 "해킹"될 가능성을 최소화한다.

>교육 중 시험 응시 시스템에서 활용하는 경험의 양을 제어해야 한다. 훈련 데이터를 무제한 샘플링하여 벤치마크에서 성과를 "구입"할 수 없어야 하고, 벤치마크는 새로운 데이터를 마음대로 생성할 수 있는 작업을 피해야 한다.

> 가정한 사전 세트를 명확하고 완전하게 설명해야 한다. 모든 작업에는 사전 작업이 포함되지만 오늘날 AI 평가에 사용되는 많은 과제들은 선입견이 내포되어 있다. 그리고 암묵적으로 숨겨진 사전 지식의 존재는 종종 인간이나 기계에 불공평한 이점을 줄 수 있다.

> 인간과 기계 모두에게 공평한 효과가 있을 것이다. 그리고 인간이 보유하는 것과 동일한 사전 지식(예: 핵심 지식)만 가정하고, 인간 크기의 연습 시간 또는 훈련 데이터만 요구한다.

일반적인 AI 평가를 위한 이러한 권장사항은 구현하기 위한 구체적인 노력 없이는 완성되지 않을 것이다. 3장에서 우리는 우리의 초기 시도를 제시한다.

# 3. 벤치마크 제안 : ARC 데이터 세트

3장에서 우리는 2.2에 정의된 일반적인 지능의 종류에 대한 벤치마킹 역할을 하기 위한 데이터 세트를 사용하는 추상화 및 추론 말뭉치(ARC)를 소개한다. ARC는 가능한 많은 2.3의 권장사항을 통합하도록 설계되었다.

## 3.1 설명 및 목표

### 3.1.1 ARC가 무엇인가?

ARC는 프로그램 합성 벤치마크 또는 심리측정학 지능 테스트로서의 일반적인 인공지능 벤치마크로 볼 수 있다. 이것은 인간과 같은 형태의 일반 유체 지능을 모방하는 것을 목표로 하는 인공 지능 시스템과 인간 모두를 대상으로 한다. 이는 1930년대 전형적인 IQ 테스트 형식인 Raven의 진행 행렬과 형식이 다소 유사하다.

ARC는 다음과 같은 최상위 목표를 가지고 있다.

> AI 평가를 위해 이런 시험을 사용한 사전 지식에서 발견된 문제를 다루면서 정신계 지능 시험에 가깝게 형식을 유지한다(3.1.3 참조). 이로인해 인간과 기계 모두가 접근하기 쉬워졌다. 

> 특정 업무 기술보다는 개발자 인식 일반화를 측정하는 데 집중해야 한다. 그리고 평가 세트에 새로운 작업만 포함해야한다(테스트 응시자의 개발자는 알 수 없음).

> 질적으로 "넓은" 형태의 일반화를 측정하는 데 초점을 맞줘서 매우 적은 예를 사용하여 시험 응시자가 이해해야 하는 매우 추상적인 작업을 특징으로 한다(1.3.2 참조).

> 각 작업에 대해 일정한 양의 훈련 데이터만 제공하고, 인위적으로 새로운 데이터를 생성하는 데 자신을 잘 빌려주지 않는 작업만 특징으로 선택 함으로써 경험을 정량적으로 제어한다.

> 가정한 사전 지식의 전체 세트를 명시적으로 설명한다(3.1.2 참조). 그리고 타고난 인간의 사전 지식에 가까운 사전 지식만을 요구함으로써 인간과 기계 사이의 공정한 일반적인 지능 비교를 가능하게 한다(2.3.2 참조).

ARC는 교육 세트와 평가 세트로 구성된다. 과제의 400개는 교육 세트에, 600개는 평가 세트에 포함된다. 평가 세트는 공개 평가 세트(400개 작업)와 비공개 평가 세트(200개 작업)로 추가로 분할된다. 작업 데이터는 [github.com/fchollet/ARC](github.com/fchollet/ARC) 에서 확인할 수 있다. 
...
작업에 대한 테스트 예제의 입력 그리드뿐만 아니라 평가 과제를 해결할 때, 시험 응시자는 작업에 대한 교육 예제에 접근할 수 있다(입력 그리드와 출력 그리드 모두). 시험 응시자는 각 시험 예제의 입력 그리드에 해당하는 출력 그리드를 스스로 구성해야 한다. "출력 그리드 구성"은 처음부터 완전히 완료된다. 즉, 시험 응시자는 출력 그리드의 높이와 너비를 결정해야 한다. 시험 응시자가 과제에 대한 모든 시험 예제에 대해 정확한 정답을 도출할 수 있다면 과제가 성공적으로 해결된다(성공의 이항 측정). 작업의 각 테스트 예에 대해 시험 응시자(인간 또는 기계)는 3개의 시행이 허용된다. 평가판 사용 후 수신된 유일한 피드백은 이진(정답 또는 오답) 상태이다. ARC의 지능 시스템 점수는 성공적으로 해결할 수 있는 평가 세트의 작업 비율입니다. 이는 결정적으로 시험 응시자나 개발자가 평가 세트에 포함된 작업에 대한 사전 정보를 가지고 있지 않았을 것으로 가정한다. ARC는 I.3.2에 정의된 "개발자 인식 일반화"를 측정하려고 한다. 전용 평가 세트의 존재는 우리가 공공 경쟁 환경에서 이것을 엄격하게 집행할 수 있게 한다.

교육 데이터가 검증 세트에서 성공하기 위해서 꼭 필요하지 않지만 시험 응시자는 훈련 세트 전체에 접근할 수 있는 것으로 가정한다. 모든 작업은 고유하며 3.1.2에서 설명한 사전 지식 이외의 지식을 가정하지 않는다. 일반적인 인간은 사전 지식 훈련 없이도 주로 AI 시스템 개발자를 위한 개발 검증 세트 또는 인간 시험 응시자를 위한 모의 시험으로 기능하도록 설정된 훈련의 목적과 같은 대부분의 ARC 평가 세트를 해결할 수 있다. 또한 알고리즘에 핵심 지식의 이전 내용을 친숙하게 만드는 방법으로도 사용될 수 있다. 우리는 훈련 세트에 대한 연습이 테스트 세트의 인간 성과를 증가시킬 것으로 기대하지 않는다.

### 3.1.2 중심 사전 지식

지능 테스트에는 사전 지식이 포함된다. ARC는 가정한 사전 지식을 명시적으로 나열하여 자체 가정을 통제하려고 한다. 그리고 이러한 사전 정보가 아닌 정보(예: 언어와 같은 습득된 지식)에 의존하지 않는다. ARC 사전 지식은 인간의 지능과 인공지능을 비교할 수 있는 공정한 근거를 제공하기 위해서 2.3.1의 권고사항에 따라 핵심 사전 지식과 최대한 근접하도록 설계되었다.

ARC가 가정한 핵심 지식은 다음과 같다.
...
#### 사전 지식의 기본 기하학 및 체계적인 분류

ARC 작업은 다양한 사전 지식의 기본 기하학 및 체계적인 분류 개념을 특징으로 한다.

> \- 선, 직사각형 모양(복잡한 도형보다 정규 도형이 더 잘 나타남)           
  \- 스케일업 또는 스케일다운, 탄력적인 왜곡 형성         
  \- 선, 연결 점, 직교 예측         
  \- 개체 복사, 반복        
  \- 회전, 편환, 대칭(그림 11 참조)
...
우리는 I.3.4에서 기존의 심리측정학 지능 테스트(또는 "IQ 테스트")를 사용하는 것이 AI 평가를 위한 건전한 기반이 되지 않는 이유를 지적했다. ARC는 전통적인 IQ 테스트와 근접하게 유지되지만 근본적인 면에서 그들과 다르다. 우리는 이러한 차이가 AI 평가의 맥락에서 심리 측정 지능 테스트의 단점을 해결한다고 주장한다.

> 몇몇 정신측정학 지능 테스트와는 달리, ARC는 결정화된 지능이나 결정화된 인지 능력을 평가하는 데 관심이 없다. ARC는 추리와 추상화에 중점을 두고 일반적인 형태의 유체 지능만 평가한다. ARC는 언어, 실제 사물의 그림 또는 실제 상식을 포함하지 않는다. ARC는 사전 지식의 핵심과 가까운 지식만 포함하려고 하고, 직무별 실습을 통해 인간이 습득해야 할 지식을 회피한다. ARC가 개발자 인식 일반화를 평가하려고 할 때, 

> 고유한 ARC 평가 세트에 포함된 작업은 시험 응시 시스템 개발자가 알지 못하는 것을 의미한다. 이는 개발자가 직접 작업을 해결하고 프로그램 형태로 솔루션을 하드 코딩하는 것을 방지한다. 이것은 개인 평가 세트가 존재하여 경기 설정에서 엄격하게 시행될 수 있다.

> ARC는 일반적인 정신 지능 테스트(작업 간 중복이 제한된 수백 개의 고유 작업)보다 작업 다양성이 더 크다. 이는 하드 코딩된 작업별 솔루션이 공공 평가 세트에서도 개발자를 위한 실용적인 지름길을 나타낼 가능성을 감소시킨다.

> C-Test의 작업과 달리, ARC 작업은 대부분 프로그래밍 방식으로 생성되지 않는다. 우리는 정적 "마스터" 프로그램의 프로그래밍적 세대를 약점으로 인식한다. 이것은 단순한 프로그램, 시험 개발자에 의해 기록되어야 했으므로 작업들 사이에 공유되는 생성 프로그램들을 역설계하는 것 만으로도 충분하다. 이를 통해 수동 작업 생성은 작업 다양성을 증가시키고 시험 해결에서 광범위한 능력의 필요성을 우회하는 데 사용될 수 있는 예기치 않은 바로 가기의 존재 위험을 감소시킨다.

### 3.1.4 ARC에 대한 솔루션이 어떤 모습이며, AI 애플리케이션에 어떤 영향을 미칠 수 있는가

우리는 ARC가 인간에 의해 완전히 해결된다는 것을 발견했다. 많은 ARC 테스트 작업이 지능적으로 어렵지만, 인간 수험생들은 연습이나 구두 설명 없이도 대부분의 과제를 첫 시도에서 풀 수 있는 것으로 보인다. ARC에 포함된 각 작업은 세 명의 높은 IQ 인간 그룹 중 적어도 한 명(서로 의사 소통하지 않음)에 의해 작업 실현 가능성의 입증을 성공적으로 해결했다. 우리는 미래에 통계적으로 유의한 양의 인간 테스트 데이터를 수집하여 ARC에 대한 인간 성능을 더욱 조사할 수 있기를 바란다(특히 CHC 인지 능력과 ARC 성능 간의 관계에 관한 것). 

결정적으로 ARC는 광범위한 일반화와 적은 학습 횟수에 중점을 두고, 평가 세트가 교육 세트에 나타나지 않는 작업만 포함하고 있기 때문에 기존의 Machine Learning 기술(Deep Learning 포함)에 의해 접근 가능한 것으로 보이지 않는다.

이 문제를 해결하면 ARC는 프로그램 합성 벤치마크를 가장 잘 이해할 것이다. 프로그램 합성은 높은 수준의 사양을 충족하는 프로그램 생성에 관심이 있는 AI의 하위 분야이다. 이것은 ARC 형식으로 프로그램에 대한 입력과 출력에 의해 제공되는 경우가 많다. 

가상 ARC 해결사는 입력 그리드를 해당 출력 그리드로 변환하는 후보를 생성하기 위해 작업의 시연 예시를 사용하는 프로그램 합성 엔진의 형태를 취할 수 있다. 이는 대략적으로 아래와 같다. 

>  ARC 작업의 정확한 집합은 의도적으로 공식적으로 정의될 수 없기 때문에 ARC 작업에 대해 가능한 모든 솔루션 프로그램을 표현할 수 있는 영역별 언어(DSL) 개발부터 시작해야 한다. 작업의 공간은 핵심 지식만을 포함하는 ARC 쌍의 관점에서 표현 가능한 것으로 정의되기 때문에 DSL 개발은 어려울 수 있다. 충분히 추상적이고 조합 가능한 프로그램 형태로 3.1.2의 사전 핵심 지식을 하드 코딩해야 일종의 "인간다운 추리 DSL"의 기본 기능 역할을 할 수 있을 것이다. 우리는 이 특정 하위 문제를 해결하는 것이 일반적인 AI 진행에 매우 중요하다고 생각한다.

> 작업이 주어졌을 때, DSL을 사용하여 입력 그리드를 해당 출력 그리드로 전환하는 일련의 후보 프로그램을 생성해야 한다. 이 단계는 이전에 다른 ARC 작업에서 유용하다고 입증된 하위 프로그램을 재사용하고 재결합한다. 

> 프로그램 단순성 또는 프로그램 가능성과 같은 기준에 따라 이러한 프로그램 중 상위 후보자를 선택한다(이 기준은 ARC 교육 세트를 사용하여 이전에 생성된 솔루션 프로그램에 대해 훈련될 수 있음). 우리는 훈련 쌍에서 작동하는 가장 단순한 프로그램을 선택하는 것만으로는 테스트 쌍이 잘 일반화될 것으로 기대하지 않는다.

> 상위 세 명의 후보자를 사용하여 테스트 예제에 대한 출력 그리드를 생성해야 한다.

인간 수준 ARC 해결사의 존재는 일반적으로 인간 수준이 필요로 하는 종류의 광범위한 인간 관련 작업을 수행할 수 있는 상황에서만 AI를 프로그래밍할 수 있는 능력을 나타낼 수 있다고 우리는 주장한다. 우리는 (ARC와 유사한) 심리측정학 지능 테스트에서의 인간 성과가 모든 인간 인지 작업에 걸친 성공을 예측한다는 점에 주목한다. ARC 해결사와 인간의 지능은 모두 사전 지식과 같은 지식 위에 세워질 것이기 때문에, ARC 해결사의 적용 범위는 인간 인식의 적용 범위에 가까울 것이다. 이를 통해 이러한 해결사를 실용적이고, 가치 있고(유용한 인간 관련 문제를 해결할 수 있음), 쉽게(인간의 상황을 쉽게 이해하고 인간의 기대와 일치하는 행동을 만들 수 있음) 만들 수 있다.

우리의 주장은 추측이 많으며 완전히 부정확한 것으로 판명될 수도 있다. 특히 ARC가 지능적이지 않은 지름길에 예기치 않은 취약성을 가진 것으로 판명될 경우 더욱 그럴 것이다. 

## 3.2 약점 및 향후 개선 사항

ARC는 최종 솔루션이 아니라 진행 중인 작업이라는 점을 유념해야 한다. 이것은 2.3.2에서 설명한 모든 요구 조건에 맞지 않고, 다음과 같은 여러 약점을 갖추고 있다.

> **일반화가 정량화되지 않았다.**       
  ARC는 "지역 일반화", "극한 일반화"와 반대로 "광범위한 일반화"를 측정하도록 명시적으로 설계되었다. 광범위한 일반화는 테스트 세트 또는 각 작업의 일반화 난이도에 대한 정량적 측정을 제공하지 않는다(독립적으로 고려됨). 우리는 이 문제가 나타내는 일반화 난이도를 추정하기 위해 문제(많은 인간 주제에 대해 고려됨)에 대한 인간 성과를 사용하여 이 문제를 경험적으로 해결하기 위한 향후 작업을 수행할 계획이다. 우리는 특히 작업에서 인간의 성과를 2.2에서 제안된 일반화 난이도의 AIT 척도와 상관시키려 할 것이다(이런 근사치는 ARC 해결사 프로그램에서 발전을 이루면서 사용할 수 있어야 한다). 높은 상관 관계, 상관 관계 부족을 찾는 것은 공식 검증 또는 무효화 정도를 제공할 것이다.
  
> **테스트 유효성이 설정되지 않았다.**       
  타당성은 다른 비시험 활동에서의 성과에 관한 시험 성능의 예측성을 나타낸다. ARC의 유효성은 정신측정학에서 확립된 과정을 따라 사람에 대한 대규모 표본 크기 통계 연구를 통해 조사되어야 한다. AIARC의 문제를 해결하면, 우리는 또한 ARC 성능이 다양한 작업에 걸쳐 실제 유용성으로 얼마나 잘 변환되는지 연구할 수 있을 것이다.

> **데이터 세트의 크기와 다양성은 제한될 수 있다.**      
  ARC는 총 1,000개의 작업에만 지원하고, 많은 작업에 걸쳐 개념적으로 겹치는 부분이 있을 수 있다. 이로 인해 ARC는 지능을 사용하지 않고도 작업을 해결할 수 있는 바로 가기 전략에 잠재적으로 취약해질 수 있다. 우리는 이러한 지름길을 만들기 위한 크라우드소스의 방법으로 공공 AI 대회를 운영할 계획이다(지름길이 존재하는 경우, 경쟁 환경에서 빠르게 발생해야 한다). 그리고 이러한 지름길에 대한 잠재적인 취약성을 완화하고, 향후에도 ARC에 새로운 작업을 계속 추가할 계획이다. 
  
> **평가 형식이 지나치게 폐쇄적이고 이진 형식을 취한다.**          
  세밀성이 결여된 평가 작업에서 수험생의 점수는 0 또는 1이다. 더 나아가 실제 문제 해결은 종종 가설이 시험자에 의해 공식화된 다음 경험적으로 시험되는 대화형 프로세스의 형태를 취한다. ARC에서 이 접근방식은 시험 응시자가 직무의 각 시험 예에 대해 3개의 시험을 허용하기 때문에 어느 정도 가능하지만 이러한 형식은 지나치게 제한적이다. 더 나은 접근방식은 시험 응시자가 작업에 대한 예제 생성기와 동적으로 상호작용하도록 할 수 있다. 시험 응시자는 새로운 시험 입력을 원하는 대로 요청할 수 있을 것이다. 이는 테스트 입력을 위한 솔루션을 제안하고 들의 솔루션에 대한 피드백을 받는 과정을 시험 응시자가 신뢰할 수 있게 정답을 제시할 수 있을 때까지 반복한다. 과제에 대한 시험 응시자의 점수는 새로운 입력에 대한 정확한 솔루션을 신뢰성 있게 생성할 수 있을 때까지 필요한 피드백의 양을 측정하는 것이 될 것이다. 이는 입력 생성기가 커리큘럼을 제어하는 방법이고 2.2에서 정의된 지능보다 직접적인 측정치를 나타낸다. 
  
> **핵심 사전 지식은 잘 이해되지 않을 수 있으며 ARC에서 잘 포착되지 않을 수 있다.**            
  ARC의 중심 개념은 타고난 인간의 사전 지식에만 의존하며 상당한 양의 습득된 지식을 특징으로 하지 않는 것이다. 그러나 타고난 인간의 사전 지식의 정확한 본질은 여전히 열린 문제이고, 이러한 사전 지식이 ARC에 올바르게 포착되었는지 여부가 불분명하다.
  
## 3.3 가능한 대안

ARC는 가능한 많은 (3.3에 나열된)지침을 구체화하는 인간과 유사한 일반 지능 벤치마크를 만들기 위한 하나의 시도일 뿐이다. ARC는 정신측정학 지능 테스트의 형식에 매우 근접하지만 다른 많은 가능한 접근법이 탐구될 수 있다. 이 부분에서 우리는 몇가지 제안을 제공한다.

### 3.3.1 광범위한 일반화를 측정하기 위한 기술 벤치마크 용도 변경

우리는 1.3.5에서 보드 게임과 비디오 게임에서 인간의 기술을 능가하는 시스템을 개발하는 데 있어 AI 연구 커뮤니티의 지속적인 관심이 있었다는 것을 확인했다. 우리는 그러한 기술 테스트를 지능 테스트로 재배치할 것을 제안한다.

게임 X의 해결에 관심이 있는 AI 개발자를 생각해 보자. AI가 X의 사례에 대해 훈련되는 동안 평가 중재자는 X의 여러 변형(X<sub>1</sub>, X<sub>2</sub>, X<sub>n</sub>)을 생성한다. 이러한 대안 게임들은 X에 대한 의미 있는 일반화 난이도를 나타내도록 설계될 것이다 (2.2에서 정의됨). X의 사례(예: X의 게임 레벨)에서 최적인 가장 단순한 게임 플레이 프로그램은 X<sub>i</sub>에서는 최적이 아닐 것이다. 이러한 대체 게임들은 단지 X의 "새로운 레벨"만은 아닐 것이다. 지역 일반화와는 반대로 광범위한 일반화를 측정하기 위해 관련성은 있지만 다른 게임 플레이가 있을 수 있다. 이러한 대체 게임들은 개발자 인식 일반화를 측정하기 위해  AI 개발자들에게 알려지지 않은 채로 남아있을 것이다. CoinRun, Obstacle Tower과 같이 평가 환경이 대체 게임이 아닌 경우가 있다. 그러나 이러한 경우는 동일한 게임의 수준(지역 일반화 또는 알려진 미지의 일반화에 대한 일반화)만 해당한다. 

AI는 X에 대한 훈련이 완료되면 X<sub>1</sub>, X<sub>n</sub>을 해결 것을 배우는 임무를 맡게 될 것이다. 평가 점수는 특정 기술 한계점에 도달하기 위한 각 대안 게임에서 요구되는 경험의 양을 측정하는 척도가 될 것이다. 이는 각 대체 게임에 의해 대표되는 일반화 난이도에 의해 조절된다. 그런 다음 이러한 AI의 일반적인 지능의 척도는 많은 수의 다른 소스 게임 X에 대한 평가 점수의 평균이 될 것이다.

DotA2 게임을 예를 들어 보자. DotA2에서 훈련된 AI는 그것이 같은 장르에서 새로운 게임을 배우는 것을 배울 수 있는 효율성을 측정함으로써 평가될 수 있다. 더 간단한 예시로는 16개의 특정 DotA2 문자에 대해 훈련된 AI는 사전에 학습되지 않은 새로운 문자의 집합을 마스터하는 것을 배울 수 있는 효율성을 측정함으로써 평가될 수 있다.

### 3.3.2 개방적이고, 적대적 또는 협업적인 접근 방식

우리는 3.2에서 평가 과제를 수동으로 작성해야 하는 주요 제한사항 중 일부를 지적했다. 이것은 일반화의 어려움을 공식적으로 통제하기 어렵게 만드는 노동집약적인 과정으로 잠재적으로 작업량이 적을 수 있고, 쉽게 확장되지 않는다(군중을 대상으로 하는 작업이 부분적으로 이 문제를 해결할 수 있다). 벤치마크가 개발자 인식 일반화를 측정한다는 것을 보장하기 위해 실질적으로 새로운 작업의 지속적인 공급이 필요하다는 점을 감안할 때 다양성과 확장성 포인트는 특히 중요하다.
...
교사 프로그램은 주어진 학생의 새로움과 흥미에 대한 과제 생성을 최적화할 수 있고(과제는 새롭고 도전적이지만 학생은 여전히 해결할 수 있어야 한다), 학생들은 점점 더 어려운 과제를 해결하는 법을 배우도록 진화한다. 또한 교사 프로그램이 학생들의 학습 효율을 최적화하도록 구성될 수 있기 때문에 커리큘럼 최적화에도 유리하다. 이 아이디어는 "언제든지 실행 가능한 지능 테스트"와 PIET 시스템과 유사하다.

생성된 작업의 공간이 시간이 지남에 따라 충분한 복잡성과 새로움을 유지하도록 하기 위해 교사 프로그램은 외부 소스로부터 현실세계와 같은 정보를 얻어야 한다(압축할 수 없는 복잡성을 특징으로 한다). 이러한 외부 복잡성의 원천으로 인한 설치 과정은 매우 개방적입니다. 인간 관련 작업을 부분적으로 모방하는 새로운 작업을 생성하는 교사 프로그램은 결과 학생 프로그램을 실제 인간 관련 문제로 옮길 수 있는 지능의 형태로 안내할 수 있다는 추가적인 이점을 가질 수 있다.

# 결론

우리는 일반적인 인공지능에 대한 연구는 아직 걸음마 단계이지만, AI 시스템이 가지고 있는 지능을 특성화하고 측정하는 문제에 대한 결정적인 해결책만을 제공했다는 인상을 전달하고 싶지 않다. 오히려 다음과 같은 아이디어를 중심으로 구성된 지능 정의하고 평가하는 새로운 관점을 도입했다.

> 지능은 학습 시스템이 이전에 알려지지 않은 작업에서 경험과 선행 기술을 기술로 바꾸는 효율성이다.

> 따라서 지능의 척도는 사전 지식, 경험, 일반화의 어려움을 설명해야 한다.

> 모든 지능은 적용범위에 상대적이다. 두 개의 지능형 시스템은 공유 범위 내에서 유사한 이전 시스템을 공유하는 경우에만 의미 있게 비교될 수 있다.

> 따라서 일반 AI는 인간의 지능에 대해 벤치마킹되어야 하며 유사한 지식 집합(예: 핵심 지식)에 기초해야 한다.

또한 우리는 알고리즘 정보 이론(2.2 참조)에 기초한 새로운 형식주의를 제공하고, 이를 통해 일반 지능 벤치마크(2.3.1, 2.3.2 참조) 개발을 위해 따라야 할 일련의 구체적인 지침뿐만 아니라 이러한 아이디어에 대해 엄격하고 정량적으로 추론한다.

우리의 정의, 공식 프레임워크, 평가 지침은 지능의 모든 면을 포착하지는 못하지만, 설명적이거나, 철저하거나, 합의하는 것이 아니라 실행 가능하고, 설명 가능하며, 수량화할 수 있도록 개발되었다. 이는 지능에 대한 다른 관점을 무효화하려는 것이 아니다. 오히려 2.2.3에서 요약된 것처럼 넓은 AI와 일반 AI에 대한 연구를 안내하는 유용한 목적 함수의 역할을 하도록 설명한다. 

우리는 무한한 경험과 무한한 사전 지식이 주어지기 때문에 일반적인 AI에 관심이 있는 AI 커뮤니티의 일부가 도전적인 작업에서 원시 기술을 달성하고자 하는 오랜 지속적 추세에서 벗어나기를 바란다.

우리의 생각을 기초로 하고 다른 사람들이 그것을 기반으로 할 수 있게 하기 위해서 추상화 및 추론 언어 집합 또는 ARC와 같은 실제 벤치마크를 제공한다. 

> ARC는 지능 테스트의 범위, 사전 지식, 경험에 대해 제어를 진행한다. 이 과정에서 모든 시험 과제는 새로운 것이어야 하며(기술보다는 새로운 과제를 이해하는 능력을 갖추어야 한다) 모든 시험 응시자가 공유하는 명시적인 사전 세트를 가정해야 한다.

> ARC는 선천적으로 인간이 소유했던 이전과 동일한 핵심 지식을 명시적으로 가정한다.

> ARC는 인간에 의해 충분히 해결될 수 있으나 Deep Learning을 포함한 현재의 Machine Learning 기법으로 의미 있게 접근할 수는 없다.

> ARC는 인간과 유사하고 광범위한 일반화가 가능한 알고리즘을 개발하는 데 관심이 있는 AI 연구자들에게 흥미로운 기반을 제공할 수 있다. 또한 우리가 같은 사전 지식으로 추정하는 것과 같이 인간의 지능과 기계 지능을 비교할 수 있는 방법을 제공할 수 있다.

ARC는 여전히 3.2에서 명시된 약점이 존재한다. 우리는 연구 기반이자 기계 지능과 인간 지능을 위한 공동 벤치마킹 장소인 향후 데이터 세트를 더욱 개선할 계획이다. 

우리의 메시지 성공의 척도는 일반 AI에 관심 있는 커뮤니티의 일부의 관심을 기술 테스트에서 인간을 능가하는 것에서 벗어나 프로그램 합성, 핵심 사전 지식, 커리큘럼 최적화를 통해 인간과 유사한 광범위한 인지 능력의 개발을 조사하는 쪽으로 돌리는 능력일 것이다.
