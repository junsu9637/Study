...
**변수 초기화**
```Java
int age = 22; //age라는 이름의 int 타입 변수를 선언하고, 22로 초기화
```
...
단, 예외적으로 float에서 double로의 형변환에서는 0을 오른쪽에 추가한다. 그리고 double가 float보다 비트가 훨씬 많기 때문에 다음과 같이 실수간의 형변환 과정에서 문제가 자주 발생한다.

```
double 0 010101010 1010100000000000000000000000000001

(float로 형변형)

float 0 0101010 101010000000000000
```
