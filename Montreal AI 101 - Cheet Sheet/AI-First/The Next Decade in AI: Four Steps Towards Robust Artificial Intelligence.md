...
```
아쉽게도 텍스트(비디오는 말할 것도 없고)에서 풍부한 인지 모델을 도출하기 위해 일하는 사람은 거의 없다. 시간이 지남에 따라 발전하는 사건을 기술하는 것 파수파트와 리앙의 논문 (Pasupat & Liang, 2015)은 테이블 너머로 훑어볼 수 있는 프로그래밍 가능한 쿼리로 문장을 구문 분석하려고 시도한다.
```
